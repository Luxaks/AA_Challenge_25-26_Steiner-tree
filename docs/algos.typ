...
Abordarea de tip programare dinamică implementată este cea a lui *Fuchs*, optimizată față de formularea clasică a algoritmului lui *Dreyfus-Wagner*. Această abordare urmărește construirea unui arbore Steiner optim prin combinarea unor soluții parțiale corespunzătoare submulțimilor de terminale. Complexitatea temporală a algoritmului este exponențială în numărul de terminale k, datorită faptului că pentru fiecare subset de terminale se calculează soluții parțiale, iar complexitatea de memorie este $O(n * 2^k)$, unde n este numărul total de noduri.
